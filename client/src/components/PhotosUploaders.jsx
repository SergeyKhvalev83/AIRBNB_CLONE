...
const PhotosUploaders = ({ addedPhotos, onChange }) => {
  ...
  const removePhotoHandler = (ev, fileName) => {
    ev.preventDefault();
    onChange(addedPhotos.filter((each) => each !== fileName));
  };

  const mainPhotoHandler = (ev, fileName) => {
    ev.preventDefault();
    onChange([fileName, ...addedPhotos.filter((each) => each !== fileName)]);
  };
  ...
  return (
    <div>
      ...
      <div className="mt-2 grid gap-2 grid-cols md: grid-cols-4 lg:grid-cols-6">
          ...
          addedPhotos.map((eachPhotoName) => (
            ...
            >
              ...
              <button
                ...
                onClick={(ev) => removePhotoHandler(ev, eachPhotoName)}
              >
                <svg
              ...
              <button
                ...
                onClick={(ev) => mainPhotoHandler(ev, eachPhotoName)}
              >
                {eachPhotoName === addedPhotos[0] ? (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
